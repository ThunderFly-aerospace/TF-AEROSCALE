...
class SPIWraper:
    def __init__(self,cs_pins):
        ...
        self.mcp._spi_settings.bit_rate=1000000
        self.mcp._spi_settings.mode=1
    ...
    def SPI_write(self,cs,data):
        ...
        #print(data)
        tmpresult=self.mcp.spi_exchange(data, cs)
...
while 1:
    if count==1000:            
        ...
        sys.stdout.write("%s; %4.1f;\n" %(ts, 1000/(currentTime-lastTime)))
        sys.stdout.flush()
...
try:
    ...
    while 1:
        ...
        if count==1000:            
            currentTime=time.time();
            ts = datetime.datetime.utcfromtimestamp(time.time()).isoformat()
            sys.stdout.write("%s; %4.1f; %+4.3f;\n" %(ts, 1000/(currentTime-lastTime), channel1))
            sys.stdout.flush()
            lastTime=currentTime
            count=0
        count=count+1    
