...
teeth = 77;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
...
overal_d = 130;     // top cylinder diameter
...
rotor_center_h=rotorBearingNeck_h()+rotor_nut_height-rotor_rpm_hole;
rotor_center_d=26;
rotor_center_hole_d=rotorBearingNeck_d()+1;
rotor_center_top_h=5;
rotor_screw_d=3;
rotor_screw_dst=3 + 4.5 + rotor_screw_d/2;

rotor_rpm_d=40;
rotor_rpm_inner_d=rotor_center_hole_d+8;
rpm_sensor_count=16;
    ...
    difference(){
        union(){
            ...
            translate([0, 0, pulley_t_ht])
                cylinder(d1 = overal_d- overal_rank, d2 = overal_d, h = overal_ht - pulley_t_ht, $fn = draft ? 50 : 200);


            translate([0, 0, pulley_t_ht])
                cylinder(d = overal_d- overal_rank, h = 3, $fn = draft ? 50 : 200);
            
            translate([0,0,overal_ht])
                cylinder(d1 = rotor_rpm_d, d2=rotor_center_d, h = rotor_center_h-overal_ht);
                
            cylinder(d = rotor_rpm_d, h = overal_ht+0.01);
        }
        
        //vykuchání rolničky se zuby senzoru
        translate([0,0,-0.001])
        difference()
        {
            union()
            {
                translate([0,0,overal_ht])
                    cylinder(d1 = rotor_rpm_d-6, d2=rotor_center_hole_d, h = rotor_center_h-rotor_center_top_h-overal_ht);
                cylinder(d = rotor_rpm_d-6, h = overal_ht+0.01);
            }
            for (i=[0:rpm_sensor_count]) 
                rotate([0, 0, (360/rpm_sensor_count)*i])
                        linear_extrude(height = 6)
                            polygon(points=[[rotor_rpm_inner_d/2,0],
                                            [rotor_rpm_d/2, 0],
                                            [rotor_rpm_d/2*cos(360/rpm_sensor_count/2),rotor_rpm_d/2*sin(360/rpm_sensor_count/2)],
                                            [rotor_rpm_inner_d/2*cos(360/rpm_sensor_count/2),rotor_rpm_inner_d/2*sin(360/rpm_sensor_count/2)]]);
        }
                
        //dírypro přišroubování rotoru        
        for(i=[-1,1])
            translate([i*rotor_screw_dst,0,0])
            {
                nut_bottom=rotor_center_h-rotor_center_top_h+0.5;
                nut_poc_h=2.9;
               
                //šroubek
                cylinder(h=nut_bottom+0.1,d=rotor_screw_d, $fn = 200);
                translate([0,0,nut_bottom+nut_poc_h+0.1])
                    cylinder(h=10,d=rotor_screw_d, $fn = 200);
                    
                //maička s kapsou
                translate([0,0,nut_bottom])
                hull()
                {
                    cylinder(d = M3_nut_diameter, h = nut_poc_h, $fn = 6);
                    translate([i*50,0,0])
                        cylinder(d = M3_nut_diameter, h = nut_poc_h, $fn = 6);
                }
            }
        ...
        //matička rotoru
        translate([0, 0, rotor_center_h-rotor_center_top_h+0.2])
            union(){
                cylinder(d = M3_nut_diameter-0.05, h = rotor_center_top_h, $fn = 6);
                for(r = [1:6])
                    rotate([0, 0, r*360/6])
                        translate([M3_nut_diameter/2, 0, 0])
                            cylinder(d=1, h=rotor_center_top_h, $fn=3);
            }
            
        //seříznutí pro flapping
        for(r=[0,180])
            rotate([0,0,r])
                translate([0,-rotor_flap_offset,rotor_center_h])
                    rotate([rotor_flap_angle,0,0])
                        translate([0,0,5])
                            cube([rotor_rpm_d,rotor_rpm_d,10],center=true);
        ...
        difference(){
            translate([0, 0, -0.1]) difference(){
                ...
                translate([0, 0, -0.2])cylinder(d = rotor_rpm_d, h = rotor_center_h);
            }
            translate([0, 0, 0]){
                for (i=[0:8])
                    rotate(i*360/8){
...
intersection()
{
    ...
    //cylinder(d=41,h=100,center=true);
    //cube([100,100,100]);
}